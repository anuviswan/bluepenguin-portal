...
const { formatted: discountPrice } = useCurrency(
  computed(() => currentProduct.value?.discountPrice || 0),
)
...
const discountPercentage = computed(() => {
  ...
  return Math.round(
    ((currentProduct.value.price - currentProduct.value.discountPrice!) /
...
const relatedProducts = computed(() => {
  ...
  return allProducts.value.filter((p) => p.sku !== sku.value).slice(0, 4)
})

const whatsappLink = computed(() => {
  const phone = import.meta.env.VITE_WHATSAPP_CONTACT || ''
  const message = encodeURIComponent(
    `Hi, I'm interested in ${currentProduct.value?.productName} (SKU: ${currentProduct.value?.sku})`,
  )
